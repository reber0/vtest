...
API_TOKEN = md5("dd996b71024fa97cd015f06a7f24ed30" + PASSWORD)
            ...
            $.get(url, function(data, status){
                if(data.status == 1) {
                    if (url.indexOf("dns") != -1) {
                        $('#dnslog_table').bootstrapTable('refresh');
                    }
                    if (url.indexOf("http") != -1) {
                        $('#httplog_table').bootstrapTable('refresh');
                    }
                    if (url.indexOf("xss") != -1) {
                        $('#xss_table').bootstrapTable('refresh');
                    }
                } else {
                    alert('del fail');
                }
...
<body>
    <div class="container">
        <ul id="myTab" class="nav nav-tabs">
            ...
            <li><a href="#dnslog" data-toggle="tab" onclick="$('#dnslog_table').bootstrapTable('refresh');">DNS Tools</a></li>
            <li><a href="#httplog" data-toggle="tab" onclick="$('#httplog_table').bootstrapTable('refresh');">HTTP Log</a></li>
            <li><a href="#xss" data-toggle="tab" onclick="$('#xss_table').bootstrapTable('refresh');">XSS</a></li>
            ...
            <div class="tab-pane fade" id="dnslog">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <button type="button" class="btn btn-default" onclick="$('#dnslog_table').bootstrapTable('refresh');">刷新</button>&nbsp;
            ...
            <div class="tab-pane fade" id="httplog">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <button type="button" class="btn btn-default" onclick="$('#httplog_table').bootstrapTable('refresh');">刷新</button>&nbsp;
            ...
            <div class="tab-pane fade" id="xss">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <button type="button" class="btn btn-default" onclick="$('#xss_table').bootstrapTable('refresh');">刷新</button>&nbsp;
...
@app.route('/refresh/<col>')
@auth.login_required
def refresh_data(col):
    table = ''
    if col in ["dns_log", "http_log", "xss"]:
        table = col
    else:
        return jsonify({'status': '0', 'msg': 'unkown table'})
    ...
    sql1 = "SELECT url,headers,data,ip,insert_time FROM {table} order by id desc limit {skip},{limit}".format(table=table, skip=offset, limit=limit)
    sql2 = "SELECT COUNT(*) FROM {table}".format(table=table)

    if table == "dns_log":
        rows = DB.exec_sql(sql1)
        for v in rows:
            result.append({"domain": v[0], "ip": v[1], "insert_time": v[2]})
        sql = "SELECT COUNT(*) FROM dns_log"
        rows = DB.exec_sql(sql)
